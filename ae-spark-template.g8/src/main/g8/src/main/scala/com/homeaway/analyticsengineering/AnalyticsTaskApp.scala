...
object AnalyticsTaskApp extends App with Utility {
	...
	lazy val s3fs = FileSystem.get(new URI(spark.conf.get("s3aUri") + spark.conf.get("s3Bucket")), spark.sparkContext.hadoopConfiguration)
	lazy val s3Stagefs = FileSystem.get(new URI(spark.conf.get("s3aUri") + spark.conf.get("s3StageBucket")), spark.sparkContext.hadoopConfiguration)
	...
	lazy val jobId = jc.getJobId(spark.conf.get("jobName"), spark.conf.get("objectName"), spark.conf.get("processName"))
